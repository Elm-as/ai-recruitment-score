...
- **Functionality**: Create and manage job positions with title, description, requirements, and number of openings. Archive positions instead of deleting them to preserve history. Restore archived positions when needed. Positions are automatically protected when candidates are marked as "hired" - locked positions cannot be deleted or archived to maintain hiring records integrity and prevent accidental data loss.
- **Purpose**: Establishes the evaluation criteria against which all candidates will be assessed, while maintaining a complete history of all positions. Protection prevents malicious or accidental deletion of critical hiring data after candidates have been recruited.
- **Trigger**: User clicks "New Position" button for creation, Archive icon for archiving, or "View Archives" to see archived positions. Protection is automatically applied when any candidate is marked as "hired"
- **Progression**: Click New Position → Enter job details (title, description, requirements, openings) → Save → Position appears in positions list. For archiving: Click Archive → If no hired candidates, confirm → Position moved to archives with undo option. If hired candidates exist, show protection error. For protection: Mark candidate as "hired" → Position automatically locked → Lock badge visible → Archive/delete buttons disabled with tooltip explaining protection
- **Success criteria**: Position is saved to persistent storage and can be selected for candidate evaluation. Archived positions are hidden from active view but can be restored with full data intact. Positions with hired candidates cannot be deleted or archived, display lock badge, show hired count badge, and present clear error messages when attempting protected actions. Undo functionality available for 5 seconds after archiving or deleting unprotected positions
...
- **Functionality**: Complete Stripe-integrated payment system with subscription management, payment reminders, and access control. Supports monthly and yearly billing cycles with 17% discount on annual plans. Four subscription tiers: Trial (free, 14 days), Starter (€29/mo or €290/yr), Professional (€79/mo or €790/yr), and Enterprise (€199/mo or €1990/yr). System tracks subscription status, payment history, next payment dates, and payment methods. Automated payment reminders appear 7 days before expiry and when payment is overdue. Expired subscriptions block access with clear upgrade path.
- **Purpose**: Monetizes the platform through recurring revenue with competitive pricing, ensures companies maintain active subscriptions to access the system, provides clear payment tracking and management for administrators
...
- **Hired candidate deletion attempt**: Prevent deletion with clear error message explaining that hired candidates cannot be deleted to preserve hiring records
- **Protected position deletion attempt**: Block deletion/archiving of positions with hired candidates, show error with hired count, disable action buttons visually
- **Position lock visualization**: Display lock badge and hired count badge prominently when position is protected
